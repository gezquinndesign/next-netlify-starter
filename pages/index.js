...
const recipes = [
      ...
      },
      {
        ...
        recipe:
          '1 cup fresh chard, red or swiss\n1 cup almond milk, unsweetened\n1 cup strawberries\n0.5 banana',
      },
      {
    ...
    const Recipe = ({title, recipe}) => (
      <div>
        <h1 className="entry-title wprm-recipe-name wprm-block-text-none">{title}</h1>
        ...
        <h3 className="wprm-recipe-header wprm-recipe-ingredients-header wprm-block-text-none wprm-align-left wprm-header-decoration-none wprm-header-has-actions wprm-header-has-actions">Ingredients</h3>
        <div className="wprm-recipe-ingredient-group">
          <ul className="wprm-recipe-ingredients">
            {recipe.split('\n').map((line, index) => (
              <li className="wprm-recipe-ingredient" key={index}>{line}</li>
            ))}
            </ul>
        </div>
        </div>
    ...
    // Show one recipe at a time and have a button to cycle through the recipes
    const RecipeApp = ({recipes}) => {
